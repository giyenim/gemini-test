...
시험 앱은 요청을 아껴 보낸다 (세션당 **2~3회**, 자세한 건 §8):
...
- **이탈 기록은 10건 중 1건쯤 유실된다 (도달률 약 91%).** 원인은 `sendBeacon` 이
  아니라 **사파리가 `visibilitychange` 를 제대로 쏘지 않는 것**이다 — 보내는 수단이
  아니라 보낼 계기가 오지 않는다. `visibilitychange` 와 `pagehide` 를 **둘 다** 듣는
  것이 현재 최선이고(5,200만 페이지뷰 벤치마크 기준 91%), 그렇게 하고 있다.
  더 나은 `fetchLater` 는 크롬 전용 실험 기능이라 정작 사파리에서 안 돌아 쓸 수 없고,
  `unload`/`beforeunload` 는 도달률이 더 나쁜 데다 bfcache 를 꺼서 앱을 느리게 만든다.
  **유실이 특정 문항에 몰리지 않아 "어디서 많이 이탈하나" 같은 상대 비교는 그대로
  유효하다.** 제출은 `fetch` 로 즉시 보내므로 이 문제와 무관하다.
