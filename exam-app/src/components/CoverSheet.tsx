 ...
 * 쪽 번호와 `data-page` 를 달지 않는다 — 문제 페이지가 1쪽부터 시작해야 하고
 * 쪽수 검사도 표지를 세면 안 된다. 세로 배치는 위에서 흘려 내리고 간격만 GAP 으로 잡는다.
 */
...
/**
 * 유의 사항 — 수능 표지의 꼴(띠·○ 목록)은 그대로 두고 문구만 이 시험지의 사정에
 * 맞췄다 (종이 답안지가 없고 수험 번호는 앱이 발급한다). 필적 확인 띠는 두 번째
 * 항목 안에 회색 띠로 끼워 넣고, 책 소개는 맨 끝에 붙인다.
 */
...
/**
 * 성명 칸 폭. 서명 도화지와 비율이 달라도 된다 — 획이 놓인 자리만 잘려 와서
 * (`trimToInk`) `object-contain` 으로 담기므로 어디에 얼마나 크게 쓰든 칸을 채운다.
 */
const NAME_W = 148
